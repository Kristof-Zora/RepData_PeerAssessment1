...
```{r Median-and-Mean_dayly, echo=TRUE}
...
```{r max_avg_date, echo=TRUE}
max_avg_ind<-which.max(mean_steps_ts$MEAN_ts)
max_avg_interv<-as.character(mean_steps_ts$interval[max_avg_ind])
```

So, we have that in the `r max_avg_interv`-th interval was the highest the average number of steps.
...
```{r, act_after, echo=TRUE}
head(act_without_NA)
sum(!complete.cases(act_without_NA))
```

Histogram of the total number of steps taken each day.
```{r Histogram_old-new, echo=TRUE}
...
```{r Median-Mean_old-new, echo=TRUE}
...
```{r Mean_weekday-weekend, echo=TRUE}
